...
1. Read `.claude/config.yml` → `tasks.provider` and `tasks.workflow.statuses` (semantic-key → display-name map). Resolve every `statuses.<key>` reference below through this map.
