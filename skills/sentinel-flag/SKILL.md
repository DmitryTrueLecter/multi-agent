...
7. Verify by re-reading the key you already created, never by re-invoking this skill. Re-read `<new-key>`: a status equal to the `sentinel_inbox` display name returns `flagged → <new-key>`; a `to_do` status seconds after a successful transition is read-after-write lag, so re-read the same key. Return step 6's failure string only when its two-attempt transition is exhausted. Once step 6 returns a key, this invocation carries it to completion and never creates again.
