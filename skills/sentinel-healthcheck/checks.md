# Healthcheck check catalogue

The checks `dma:sentinel-healthcheck` runs, stage by stage. Each check declares an ID (`HC-<STAGE>-NNN`), a one-sentence description, a severity (CRITICAL / WARN / INFO), a detection (the commands or reads that produce the verdict), an auto-fix (exact command, present only when the check is auto-fixable; applied only under `Fix: true`), and a manual fix (what to tell the user when auto-fix is absent or fails). New checks land here.
...
- **HC-FS-002** — Project-local dma state is consolidated under `.claude/dma/`; nothing dma-owned is left at the legacy `.claude/` root.
  ...
  - Detection: none of `config.yml`, `arch.yml`, `areas/`, `devops/`, `Justfile` exists directly under `${CLAUDE_PROJECT_DIR}/.claude/` — each belongs under `.claude/dma/`. PASS when the legacy root is clean (a migrated project, or a fresh install scaffolded straight into `dma/`). FAIL listing each legacy path still present.
  ...
  - Manual fix: move each listed path into `.claude/dma/` with `git mv`, rewrite the project-root `Justfile` import to `import '.claude/dma/Justfile'`, then re-run.
