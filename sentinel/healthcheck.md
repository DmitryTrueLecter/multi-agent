...
- **HC-WT-003** — `worktree` config covers the project's gitignored runtime artifacts.
  ...
  - Manual fix: link-safe → add the dir to `worktree.link_paths`. Install-managed → add the install command (e.g. `pnpm install --frozen-lockfile`, `npm ci`) to `worktree.setup_commands` in `${CLAUDE_PROJECT_DIR}/.claude/config.yml`.
