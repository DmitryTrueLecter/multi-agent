...
- task-mode (inverse direction): sentinel edits `.claude/areas/<area>/**` but writes to `<abs-project-root>/.claude/...` rather than the worktree copy, so the change never reaches the PR branch.
