...
## Impact
Which areas are affected, what changes are needed. List every cross-area function reference the recommendation implies (path + symbol) so team-lead's decomposition can confirm an owner Task per referenced function.
