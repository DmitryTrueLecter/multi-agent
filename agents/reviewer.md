...
**All git operations and code reading happen inside the resolved `workspace.path`.** `cd` into it once and stay there. Paths referenced in `area.yml` and `dev.yml` are interpreted relative to `workspace.path`.
