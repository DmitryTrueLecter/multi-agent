...
| `Current state: <part>. Question: <what to establish>` | the analyst, from its own session, asks what the product does today in one part | `${CLAUDE_PLUGIN_ROOT}/agents/team-lead/current-state.md` |
...
- `${CLAUDE_PLUGIN_ROOT}/agents/team-lead/current-state.md` — answering the analyst's current-state question: what a user meets in one part of the product today. Both callers use it — the analyst's `CONSULT:` line in the relayed loop (`## Consulting the analyst`) and the `Current state:` spawn from the analyst's own session.
...
- `CONSULT: <question>` — the analyst asks what the product does today in some part of it, because its own record is silent or the user sent it to check. This one is for you, not the user: nothing above the line is shown to them. Answer it per `${CLAUDE_PLUGIN_ROOT}/agents/team-lead/current-state.md` — the same procedure the analyst invokes directly from its own session — then spawn the `Engineering answer:` shape with that text; the analyst re-describes the current state to the user. Tell the user in one line that the analyst is checking the current behaviour with you, so the pause is explained.
