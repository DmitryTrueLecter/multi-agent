...
One comment = one line. If you need more, the code is poorly named — rename or extract a function with a speaking name. Module/function docstrings: optional, one line of summary if present. Forbidden: multi-paragraph docstrings, restating the spec, parameter listings (types are in the signature), section dividers (`# === Config ===`), TODOs without a tracking issue, comments that paraphrase the name of the next expression (`# Stamp the review's extras` above `session.execute(...stamp_extras...)`). A comment is justified only when the reader cannot understand *why* without it: a non-obvious invariant, a workaround for a specific external bug, a reference to a spec or ticket. Target density: 5-8% of non-blank lines in production code; above 12% is a red flag — grep the file and strip paraphrasing comments.
