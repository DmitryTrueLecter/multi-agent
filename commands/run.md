...
5. **Code Review (Task)** — `/issue-search type:task status:<statuses.code_review> label:agent:reviewer`. Launch `reviewer` agent.
6. **QA** — `/issue-search status:<statuses.qa> label:agent:qa`. Launch `qa` agent.
